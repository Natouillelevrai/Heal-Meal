        ...
        <a href="{{ route('recette.show', $recette->references) }}"
            ...
            <div class="flex flex-col">
                ...
                <div class="flex flex-col justify-around w-6/12 h-full pl-3 pr-2 z-10 bg-white relative">
                    ...
                    <div class="flex flex-col justify-between items-end w-6/12 h-full rounded-r-xl p-2 text-yellow-400"
                ...
                background-position: center;">
                        ...
                        @auth
                            <form
                                class="favForm bg-[#0E2F46] w-8 h-8 rounded text flex justify-center items-center text-xl text-green-700">
                                <input type="hidden" value="{{ $recette->id_recette }}" name="id_recette">
                                <input type="hidden" value="{{ auth()->id() }}" name="id_user">

                                <input type="hidden" name="action"
                                    value="{{ $favorites->contains('id_recette', $recette->id_recette) ? '1' : '0' }}">

                                <button class="w-full h-full cursor-pointer" type="submit">
                                    <i
                                        class="{{ $favorites->contains('id_recette', $recette->id_recette) ? 'ri-bookmark-fill' : 'ri-bookmark-line' }} icon"></i>
                                </button>
                            </form>
                        @endauth
                        ...
                        @guest
                            <div
                                class="bg-[#0E2F46] w-8 h-8 rounded text flex justify-center items-center text-xl text-white">
                        ...
                        @if ($admin)
                            <div class="h-full flex flex-col justify-evenly space-y-1">
                                <a href="{{ route('recette.show', $recette->references) }}" target="_blank"
